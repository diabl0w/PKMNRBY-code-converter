...
typedef struct {
	char name[91]; // max possible = 10 * "[female]" + 10 * "|" separators + '\0' term
	char nibU;
	char nibL;
	int keypress;
	char screen;
...
nameSetBundle initializeNameBuilder(matchSetBundle matches) {
	...
	for (int m = 0; m < matches.count; m++) { //for each match
		...
		if (matches.match[m].screen.nibL == 0  && matches.match[m].screen.nibU != 0) {
			matches.match[m].screen.nibL = matches.match[m].screen.nibU;
		} else if (matches.match[m].screen.nibL == 0  && matches.match[m].screen.nibU == 0) {
			matches.match[m].screen.nibL = 1;
		}
		...
		encodedNick.namedata[m].screen = matches.match[m].screen.nibL;
...
void hex_to_nick(char *sCode) {
	...
	for (int n=0; n < iName_count; n++) {
		...
		printf("Nickname %d (%d keypresses):\t\"%s\"\n", n + 1, name_draft.namedata[index].keypress, name_draft.namedata[index].name); 
	}
}
